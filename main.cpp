...
#define docrc(var) crc32 = (crc32 >> 8) ^ CRCTable[(crc32 ^ var) & 0xFF]

byte buffer[30];

uint shaimage(SHA256 &base, byte a, byte b, byte c, byte d, byte e)
{
    buffer[0] = a;
    buffer[2] = b;
    buffer[4] = c;
    ...
    docrc(buffer[13]);
    crc32 = ~crc32;
    buffer[14] = (crc32 >> 24) & 0xFF;
    buffer[15] = (crc32 >> 16) & 0xFF;
    buffer[16] = (crc32 >> 8) & 0xFF;
    buffer[17] = (crc32 >> 0) & 0xFF;
    int out;
    SHA256 temp(base);
    temp.Update(buffer, 30);
    temp.TruncatedFinal((byte*)&out, 4);
    return out;
}

#define EXTRA_SCAN  3        //Extra bits we want to iterate through. Should be 3 on the final run.
#define UNUSED_BITS 5        //Bits ignored from the 32 bit hash.     Should be 0 on the final run.

#define DATA_RANGE (0x100000000UL >> (UNUSED_BITS))
#define SCAN_RANGE (DATA_RANGE    << (EXTRA_SCAN))

int main()
{
    byte org[16491];

    FILE *c = fopen("dtemplate.png", "rb");
    if(!c || fread(org, 1, 16491, c) < 1) {
        perror("Error reading file dtemplate.png");
        exit(1);
    }
    fclose(c);

    c = fopen("end.png", "rb");
    if(!c || fread(buffer, 1, 30, c) < 1) {
        perror("Error reading file end.png");
        exit(1);
    }
    fclose(c);
    
    SHA256 base;
    base.Update(org, 16491);
    
    struct timeval end, start;
    uint h; ulong i; byte * arr = (byte*)&i;
    byte * data = (byte*)malloc(DATA_RANGE * 5);
    gettimeofday(&start, NULL);
    #pragma omp parallel for
    for(i = 0; i < SCAN_RANGE; i++) {
        h = __builtin_bswap32(shaimage(base, arr[0], arr[1], arr[2], arr[3], arr[4])) >> UNUSED_BITS;
        data[h*5] = arr[0]; data[h*5+1] = arr[1]; data[h*5+2] = arr[2]; data[h*5+3] = arr[3];
    }
    gettimeofday(&end, NULL);
    printf("\nComputing hashes took %luus\n", ((end.tv_sec - start.tv_sec) * 1000000 + end.tv_usec - start.tv_usec));
    gettimeofday(&start, NULL);
    c = fopen("out.bin", "wb");
    fwrite(data, 5, DATA_RANGE, c);
    fclose(c);
    gettimeofday(&end, NULL);
    printf("\nWriting to file took %luus\n", ((end.tv_sec - start.tv_sec) * 1000000 + end.tv_usec - start.tv_usec));
}
